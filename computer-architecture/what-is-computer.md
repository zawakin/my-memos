...
バベッジは、主任技術者との対立や資金不足のため、どの機械も完成させることができませんでした。 バベッジが1837年に先駆的な「Analytical Engine」を提案してから1世紀以上が経過した1941年に、最初の汎用コンピュータである「Z3」が完成したのは、それが初めてでした。
...
第二次世界大戦中、チューリングはブレッチリー・パークにある政府コード・サイファー学校(GC&CS)のために働きました。一時、彼はドイツ海軍の暗号解読を担当していた部門であるハッチ8を率いていました。ここで彼は、戦前のポーランドのボンベ法の改良や、エニグマ・マシンの設定を見つけることができる電気機械機械の改良など、ドイツの暗号解読を高速化するための多くの技術を考案しました。

チューリングは、大西洋の戦いを含む多くの重要な交戦で連合国がナチスを打ち負かすことを可能にする傍受された暗号化されたメッセージを解読する上で重要な役割を果たし、そうすることで戦争の勝利に貢献した 反事実の歴史の問題のため、ウルトラ・インテリジェンスが戦争に与えた正確な効果を推定することは難しいが、上限では、この仕事がヨーロッパの戦争を2年以上短縮し、1400万人以上の命を救ったと推定されている。

戦後、チューリングは国立物理研究所で働き、そこで自動計算エンジンを設計しました。自動計算機エンジンは、保存プログラムコンピュータのための最初の設計の一つでした。1948年、チューリングはマンチェスターのビクトリア大学のマックス・ニューマンの計算機研究所に参加し、そこでマンチェスター・コンピュータの開発を手伝い、数理生物学に興味を持つようになりました。彼は形態形成の化学的基礎についての論文を書き、1960年代に初めて観測されたベローゾフ・ジャボチンスキー反応のような振動する化学反応を予測しました。

チューリングは1952年に同性愛行為で起訴されました。1885年のラボシェール改正により、イギリスでは「重大なわいせつ行為」が刑事犯罪になることが義務付けられていました。彼は、刑務所の代わりにDESによる化学的な去勢治療を受け入れました。チューリングは1954年、42歳の誕生日の16日前にシアン化物中毒で亡くなりました。審問では、彼の死は自殺と断定されましたが、既知の証拠は偶然の毒殺と一致していることも指摘されています。

2009年には、インターネット上でのキャンペーンを受けて、イギリスのゴードン・ブラウン首相がイギリス政府を代表して「彼の扱いがひどい」と公式に謝罪しました。エリザベス2世女王は2013年にチューリングに死後の恩赦を与えた。アラン・チューリング法」とは、現在では、同性愛行為を無法化した歴史的な法律の下で注意を受けた男性や有罪判決を受けた男性を遡って恩赦した2017年のイギリスの法律の非公式な用語である。



# Church Turing thesis

[https://en.wikipedia.org/wiki/Church%E2%80%93Turing_thesis](https://en.wikipedia.org/wiki/Church–Turing_thesis)

計算可能性理論では、チャーチ・チューリング・テーゼ（計算可能性テーゼ、 チューリング・チャーチ・テーゼ、 チャーチ・チューリング思い込み、チャーチのテーゼ、チャーチの思い込み、チューリングのテーゼとも呼ばれる）は、計算可能な関数の性質についての仮説である。これは、自然数上の関数は、チューリングマシンによって計算可能である場合に限り、有効な方法で計算できると述べています。この論文は、アメリカの数学者アロンゾ・チャーチとイギリスの数学者アラン・チューリングにちなんで名付けられた。計算可能な関数が正確に定義される前は、数学者はしばしば、紙と鉛筆の方法で計算可能な関数を記述するために、効果的に計算可能という非公式な用語を使用していました。1930年代には、計算可能性の概念を形式化するために、いくつかの独立した試みが行われました。

1933年、クルト・ゲーデルは、ジャック・ハーブランドと共同で、一般的な再帰関数と呼ばれるクラスの正式な定義を作成しました。一般的な再帰関数のクラスは、すべての定数関数、射影、後継関数を含み、関数の構成、再帰、最小化の下で閉じている関数の最小のクラスです（複数の引数を持つ場合もあります）。
1936年、アロンゾ・チャーチは、λ-calculusと呼ばれる関数を定義する方法を作成しました。λ-calculusの中で、チャーチ数字と呼ばれる自然数の符号化を定義しました。自然数上の関数は、チャーチ数上の対応する関数をλ-calculusの項で表すことができれば、λ-computableと呼ばれます。
また、チャーチの仕事を知る前の1936年には、[引用が必要]アラン・チューリングは、テープ上の記号を操作することによって入力から計算を実行することができる、現在チューリング・マシンと呼ばれる機械のための理論モデルを作成した。自然数が記号の列として適切に符号化されていると、あるチューリング機械が符号化された自然数上で対応する関数を計算する場合、自然数上の関数はチューリング計算可能と呼ばれます。
ChurchとTuringは，計算可能な関数のこれら3つの正式に定義されたクラスが一致することを証明しました：関数は，Turing計算可能である場合にのみλ計算可能であり，一般的な再帰的である場合にのみλ計算可能であるということです．このことから、数学者や計算機科学者は、計算可能性の概念は、これら3つの等価なプロセスによって正確に特徴付けられると考えるようになりました。その後、計算可能性を特徴づけるための他の形式的な試みが、この信念を強化してきました（後述）。

一方、チャーチ・チューリングのテーゼは、上記の３つの計算可能な関数の正式に定義されたクラスが、効果的に計算可能な関数という非公式な概念と一致しているとしています。非公式な概念として、効果的な計算可能性の概念には正式な定義がないため、この論文はほぼ普遍的に受け入れられていますが、正式に証明することはできません。

その創始以来、我々の宇宙でコンピュータによって物理的に実現できるもの（物理的チャーチ・チューリング・テーゼ）や効率的に計算できるもの（チャーチ・チューリング・テーゼ（複雑性理論））についての記述を含む、オリジナルのテーゼのバリエーションが生まれてきました。これらのバリエーションは、チャーチやチューリングに起因するものではなく、複雑性理論やデジタル物理学の後の作業から生じます。また、このテーゼは、心の哲学（下記参照）にも意味合いを持っています。



# Computablity theory

[Computability_theory(en.wikipedia)](https://en.wikipedia.org/wiki/Computability_theory)

計算可能性理論（再帰理論としても知られる）は、数学論理学、計算機科学、計算理論の一分野であり、1930年代に計算可能関数とチューリング度の研究から始まりました。この分野は、一般化された計算可能性と定義可能性の研究を含むように拡大してきました。これらの分野では、再帰理論は証明理論や効果的記述集合理論と重複しています。

再帰理論では、以下のような基本的な問題が扱われています。

自然数上の関数が計算可能であるとはどういうことか？
計算不可能な関数は、その非計算可能性のレベルに基づいてどのように階層に分類できるのか？
知識や方法の面ではかなり重複していますが、数学的再帰理論家は相対的な計算可能性、還元性の概念、次数構造の理論を研究し、コンピュータサイエンスの分野では再帰的階層、形式的方法、形式的言語の理論に焦点を当てています。



## 計算可能集合と計算不可能集合

再帰理論は、1930年代にクルト・ゲーデル、アロンゾ・チャーチ、ローザ・ペーテル、アラン・チューリング、スティーブン・クレーネ、エミール・ポストの研究から生まれました。

研究者たちが得た基本的な結果は、有効計算という非公式な考えの正しい形式化としてチューリング計算可能性を確立した。これらの結果により、スティーブン・クレーネ(Stephen Kleene, 1952)は、「チャーチのテーゼ」(Kleene 1952:300)と「チューリングのテーゼ」(Kleene 1952:376)という2つの名前を生み出した。現在では、これらは、アルゴリズムによって計算可能な関数はすべて計算可能な関数であるとする「チャーチ・チューリングのテーゼ」という一つの仮説として考えられることが多い。当初は懐疑的であったが、1946年までにゲーデルはこのテーゼを支持するよう主張した。

"タルスキは彼の講義の中で、一般的な再帰性（またはチューリングの計算可能性）の概念の重要性を強調しています（そして私は正当だと思っています）。この重要性は、この概念で初めて興味深い認識論的概念に絶対的な概念を与えることに成功したという事実に大きく起因していると私には思われます。
効果的な計算の定義によって，数学には効果的に決定できない問題があることが最初に証明されました．チャーチ(1936a, 1936b)とチューリング(1936)は，ゲーデル(1931)が彼の不完全性定理を証明するために使った技術に触発されて，エンツシャイト問題が効果的に決定できないことを独立して証明しました．この結果は，任意の数学的命題が真か偽かを正しく判断できるアルゴリズムの手順が存在しないことを示した．

数学における多くの問題は、これらの初期例が確立された後、決定不可能であることが示された。1947年、MarkovとPostは、半群の単語問題が効果的に決定できないことを示す独立した論文を発表した。この結果を拡張して、Pyotr NovikovとWilliam Booneは1950年代に、群に対する単語問題が効果的に解けないことを独立して示した：有限に提示された群の中の単語が与えられ、その単語によって表される要素がその群の同一性要素であるかどうかを決定する効果的な手順がない。1970年、ユーリ・マチヤセビッチは（ジュリア・ロビンソンの結果を用いて）マチヤセビッチの定理を証明した。これは、ヒルベルトの10番目の問題に有効な解がないことを暗示している。決定不可能な問題のリストは，計算可能な解を持たない問題の追加の例を示している．

どの数学的構造が効果的に実行できるかという研究は、再帰数学と呼ばれることがあります； Handbook of Recursive Mathematics (Ershov et al. 1998)は、この分野で知られている多くの結果をカバーしています。
...
[Z3_(computer)(en.wikipedia)](https://en.wikipedia.org/wiki/Z3_(computer))

Z3は、コンラッド・ズースが設計したドイツの電気機械式コンピュータである。Z3は世界初のプログラム可能な完全自動デジタルコンピュータであった。プログラムコードはパンチングフィルムに保存されていた。初期値は手動で入力された:32-37

Z3は1941年にベルリンで完成したが、重要なものとはみなされず、日常的な運用には至らなかった:30, 38-39[a] Hans Georg Küssnerの研究（Küssner effect参照）に基づいて、例えば「複素行列を計算するプログラム」が書かれ、翼のフラッター問題を解くために使用された。 [b] Zuseはドイツ政府にリレーを完全に電子化されたスイッチに置き換えるための資金提供を求めたが、そのような開発は「戦争上重要ではない」とみなされたため、第二次世界大戦中に資金提供は拒否された:148 オリジナルのZ3は1943年12月21日に連合国軍のベルリン砲撃の間に破壊された。Z3はもともとV3（Versuchsmodell 3またはExperimental Model 3）と呼ばれていたが、ドイツのV兵器と混同されないように改名された 完全に機能するレプリカが、Zuseの会社であるZuse KGによって1961年に製造され、ミュンヘンのDeutsches Museumに常設展示されている:30。

Z3は1998年に原理的にチューリング完全であることが実証されたが、条件分岐を欠いていたため、Z3は計算のすべての可能な結果を推測的に計算することによってのみ、この定義を満たすことができた。

このマシンとその前任者のおかげで、Konrad Zuseはしばしばコンピュータの発明者とみなされている。
...
[Vacuum_tube(en.wikipedia)](https://en.wikipedia.org/wiki/Vacuum_tube)

真空管、電子管、バルブ（英国での使用法）、またはチューブ（北米での使用法）とは、電位差のある電極間に高真空中で電流の流れを制御する装置である。

熱電導管や熱電弁と呼ばれるタイプは、加熱された陰極から電子が熱的に放出される現象を利用しており、信号増幅や電流整流など多くの基本的な電子機能に利用されています。

一方、真空光電管のような非熱的なタイプは、光電効果によって電子を放出するため、光強度の検出などに利用されます。いずれのタイプでも、電子は管内の電界によって陰極から陽極に加速されます。

最も単純な真空管、ジョン・アンブローズ・フレミングによって1904年に発明されたダイオードは、加熱された電子放出陰極と陽極だけを含んでいます。電子は陰極から陽極へと一方向にしか流れることができません。チューブ内に1つまたは複数の制御グリッドを追加することで、グリッドまたはグリッド上の電圧によってカソードとアノード間の電流を制御することができるようになる。これらのデバイスは、ラジオ、テレビ、レーダー、録音・再生、長距離電話網、アナログコンピュータや初期のデジタルコンピュータの開発に欠かせないものであった。いくつかのアプリケーションでは、ラジオ用のスパークギャップ送信機やコンピュータ用の機械式コンピュータなどの初期の技術が使用されていたが、これらの技術を広く実用的なものにし、エレクトロニクスという学問分野を生み出したのは、熱電真空管の発明であった。

1940年代には、半導体デバイスの発明により、サーミオン管よりも小さく、効率的で、信頼性が高く、耐久性があり、安全で、経済的な固体デバイスを製造することが可能になりました。1960年代半ばから、熱電管はトランジスタに取って代わられていきました。しかし、ブラウン管（CRT）は、21世紀初頭までテレビモニターやオシロスコープの基礎となっていました。電子レンジに使用されるマグネトロン、特定の高周波アンプ、オーディオ愛好家[who?]が「より暖かい」真空管の音を好むアンプなど、いくつかの用途には今でもサーミオン管が使用されています。

電子回路バルブ/電子管のすべてが真空管というわけではありません。ガス封入管は、似たような装置ですが、ガスを含み、通常は低圧で、ガス中の放電に関連した現象を利用したもので、通常はヒーターを使用しません。



# ENIAC

[ENIAC(en.wikipedia)](https://en.wikipedia.org/wiki/ENIAC)

ENIAC (/ˈɛniæk/; Electronic Numerical Integrator and Computer)は、最初の電子的汎用デジタルコンピュータである チューリング完全であり、再プログラミングによって「大規模なクラスの数値問題」を解くことができた。

ENIACは設計され、主に米国陸軍の弾道研究所（後に陸軍研究所の一部となった）のための砲撃テーブルの計算に使用されたが、その最初のプログラムは熱核兵器の実現可能性の研究であった。

ENIACは1945年に完成し、1945年12月10日に初めて実用化された。

ENIACは1946年2月15日にペンシルバニア大学で正式に奉納され、マスコミから「巨大な頭脳」と評された 。速度とプログラム可能性の組み合わせにより、人間が 20 時間かかった軌道を ENIAC が 30 秒で計算したように、問題のために何千もの計算が可能になった(1 時間の ENIAC で 2,400 時間の人間の時間を置き換えることができる)  完成したマシンは 1946 年 2 月 14 日の夜に一般に発表され、翌日ペンシルバニア大学で正式に奉納された。1946年7月には、アメリカ陸軍兵器部隊に正式に受け入れられました。ENIACは改装とメモリのアップグレードのために1946年11月9日に停止され、1947年にメリーランド州アバディーン試験場に移されました。そこでは1947年7月29日に電源が入り、1955年10月2日午後11時45分まで連続運転されました。