...
命令セット削減型コンピュータ」という言葉によくある誤解は、単に命令を排除しただけで命令セットが小さくなるというものである 。 実際には、RISC の命令セットは年々大きくなり、現在では多くの CISC CPU よりも大きな命令セットを持つものが多い  。 21] PowerPCのようなRISCプロセッサの中には、例えばCISCのIBM System/370のような大きな命令セットを持つものもあるが、逆にDECのPDP-8は、命令の多くが複数のメモリアクセスを伴うため、明らかにCISC CPUであることがわかるが、基本命令が8個と拡張命令が数個しかない。
...
チューリングマシンとは、抽象的な機械を定義する計算の数学モデルであり、規則の表に従ってテープの上の記号を操作するものである。 モデルが単純であるにもかかわらず、任意のコンピュータアルゴリズムが与えられれば、そのアルゴリズムの論理をシミュレートできるチューリングマシンを構築することができる。

マシンは、個別の「セル」に分割された無限のメモリテープ上で動作します。その後、シンボルとユーザーが指定した命令の「有限テーブル」内の現在の位置に従って、マシンは(i)シンボルを書き込みます(例えば、有限テーブルからの数字や文字など)。(記号の消去を許可するモデルもあれば、記号を書かないモデルもある)  その後、(ii) テープを1セル左か右に移動させ(移動を許可しないモデルもあれば、ヘッドを移動させるモデルもある)  その後、(iii) (観察された記号と表の中の機械の位置によって決定される) 次の命令に進むか、計算を停止する 。]

チューリング・マシンは1936年にアラン・チューリングによって発明されたが、それを「a-machine」(自動機械)と呼んだ。 このモデルで、チューリングは2つの質問に否定的に答えることができた。(1) テープ上の任意の機械が「円形」であるかどうか（例えば、フリーズするか、計算タスクの継続に失敗するか）を判断できる機械は存在するか、同様に、(2) テープ上の任意の機械が与えられた記号を印刷するかどうかを判断できる機械は存在するか このように、任意の計算が可能な非常に単純な装置の数学的記述を提供することによって、彼は計算の性質を一般的に証明することができました、特に、エンツシャイトungsproblem（「決定問題」）の計算不可能性を証明することができました。

このように、チューリング・マシンは機械的計算の能力の根本的な限界を証明している 。任意の計算を表現することはできるが、その最小化された設計により、実際の計算には不向きである。
